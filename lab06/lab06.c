...
{	
	...
	for(i = 0;i < 9;i++){
		for(j = 0;j < 9;j++){
			...
			mat[i][j] = 0;				//in the .dat file
			else mat[i][j] = temp -'0'; //convert char to int
		}
	}
}
...
void fill (int i,int j)
{
	...
	if(mat[i][j]!=0){			//if all the elements in the puzzle is filled 
		...
		for(;k<=9;k++){	
			...
			if (flag==TRUE){    //when the last element of the whole puzzle 
				if(i==8&&j==8){ //is filled correctly,print the puzzle
					count++;
					print();
				}
				else if(j==8) fill(i+1,0);//go check next row
				else fill(i,j+1);	      //check next one element
			}
		}
		mat[i][j]=0;        //if the blank is not filled correctly,reset it as 0
	}
}
